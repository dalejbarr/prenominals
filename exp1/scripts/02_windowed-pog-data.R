...
bad_sessions <- if (file.exists("data-raw/sess-exclude.rds")) {
                  bad_sessions <- readRDS("data-raw/sess-exclude.rds")
                } else {
                  bad_sessions <- tibble(SessionID = integer(0))
                }

resp <- readRDS("data-raw/resp.rds") %>%
  anti_join(bad_sessions, "SessionID") %>%
  filter(ImgChanged %in% c("Privileged Competitor", "Privileged Contrast"),
    Clicked == "Target")
...
saveRDS(pog_subj, "data-derived/pog-window-subj-adjective.rds")
saveRDS(pog_item, "data-derived/pog-window-item-adjective.rds")

pog <- readRDS("data-derived/pog-aligned.rds") %>%
  semi_join(resp, "RespID") %>%
  filter(frame_adj >= -100, frame_n <= 250)
...
pog_subj <- pog_full %>%
  count(SessionID, Speaker, Listener, frame_n, Image) %>%
  rename(Y = n) %>%
  complete(frame_n, Image, nesting(SessionID, Speaker, Listener),
           fill = list(Y = 0)) %>%
  group_by(SessionID, Speaker, Listener, frame_n) %>%
  mutate(N = sum(Y)) %>% ungroup()

pog_item <- pog_full %>%
  count(ItemID, Speaker, Listener, frame_n, Image) %>%
  rename(Y = n) %>%
  complete(frame_n, Image, nesting(ItemID, Speaker, Listener),
           fill = list(Y = 0)) %>%
  group_by(ItemID, Speaker, Listener, frame_n) %>%
  mutate(N = sum(Y)) %>% ungroup()

saveRDS(pog_subj, "data-derived/pog-window-subj-noun.rds")
saveRDS(pog_item, "data-derived/pog-window-item-noun.rds")
