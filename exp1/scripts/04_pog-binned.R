...
pog_bin <- pog %>%
  inner_join(resp %>% select(SessionID, RespID, Speaker, Listener),
	     "RespID") %>%
  mutate(bin = floor((frame_n + 6L) / 12) * 48L) %>%
  filter(bin >= -912L)
...
message("bootstrapping 1000 datasets...")
...
saveRDS(pog_bin %>% select(-Listener), "data-derived/pog-bin.rds")
saveRDS(orig %>% select(-Listener), "data-derived/gaze-plot-full.rds")
