...
calc_interaction <- function(x) {
  x %>%
    mutate(cond = paste0(Speaker, "_", Listener)) %>%
    select(cond, bin, lratio) %>%
    spread(cond, lratio) %>%
    mutate(eff = (early_early - early_late) -
...
pog_subj_bin <- readRDS("data-derived/pog-window-subj.rds") %>%
